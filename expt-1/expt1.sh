...
for NUM_THREADS in 1 3 5 9 13 17 21 25; do
    ...
    timeout $TIMEOUT $SERVER_EXEC $HASH_TABLE_SIZE > $OUTPUT_DIR/output_server.txt &
    SERVER_PID=$!

    sleep 5

    timeout $((TIMEOUT - 10)) $CLIENT_EXEC > $OUTPUT_DIR/output_client_${NUM_THREADS}.txt 2>> $OUTPUT_DIR/client_stderr.txt &
    CLIENT_PID=$!
