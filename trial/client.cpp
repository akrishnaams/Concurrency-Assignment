...
#define ITERATION_TIME     1      // Client should get access to the response buffer every ITERATION_TIME preferably to check whether the response has arrived or not.
...
void sendRequestwaitResponse(int threadid) {
    ...
    while(running) {
        ...
        while(running) {
            ...
            oss << threadid  << " Response Lock res_available released!. NO" << "\n";
            std::cout << oss.str();
            oss.str("");
        ...
        oss << threadid  << " Response Lock res_space_available released! YES" << "\n";
        std::cout << oss.str();
        oss.str("");
